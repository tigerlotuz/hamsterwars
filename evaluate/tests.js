...
export const tests = (addEvaluator, data, separator) => [
  ...
  addEvaluator({
    ...
    checkResponse: (statusCode, body, el) => {
      ...
      return ev.status200(statusCode, el) && ev.objectHasId(body, el);
    },
  }),
  ...
  addEvaluator({
    ...
    checkResponse: (statusCode, body, el) => {
      return ev.status200(statusCode, el) && ev.isHamster(body, el);
    },
  }),
  addEvaluator({
    ...defaultEvaluator,
  ...
  addEvaluator({
    ...
    checkResponse: (statusCode, body, el) => {
      ...
      return (
        ...
        data.hamsters.filter((h) => h.wins - h.defeats === max).length ===
          body.length
      );
    },
  }),
  ...
  addEvaluator({
    ...
    resource: () => "/winners",
    checkResponse: (statusCode, body, el) => {
      const maxWins = data.hamsters
        ? data.hamsters.reduce((acc, cur) => Math.max(cur.wins, acc), 0)
        : 0;
      if (isNaN(+maxWins)) {
        console.log(
          'You have an error in the database. There is (most likely) at least one (hamster) document that does not have a value for "wins". Clean up the database and try again.'
        );
      }
      return (
        ev.status200(statusCode, el) &&
  ...
  addEvaluator({
    ...
    resource: () => "/losers",
    checkResponse: (statusCode, body, el) => {
      const maxDefeats = data.hamsters
        ? data.hamsters.reduce((acc, cur) => Math.max(cur.defeats, acc), 0)
        : 0;
      return (
        ev.status200(statusCode, el) &&
        ev.isArray(body, el) &&
