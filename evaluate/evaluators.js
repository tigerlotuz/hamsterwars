...
export const ev = {
  status200: (status, element) =>
    statusCode(status, 200, element, "Response status is not 200. (ok)"),
  status400: (status, element) =>
    statusCode(
      status,
      400,
      element,
  ...
  status404: (status, element) =>
    statusCode(status, 404, element, "Response status is not 404. (not found)"),
  ...
  isHamster: (object, element) => {
    ...
    if (
      ...
      [
        ...
        "defeats",
        "games",
      ].every((p) => object.hasOwnProperty(p))
    )
      return true;
  ...
  isMatchObject: (object, element) => {
    if (
      object &&
      ["winnerId", "loserId"].every((p) => object.hasOwnProperty(p))
    )
      return true;
  ...
  hasValue: (value, prop, hamsterArray, element, message) => {
    if (hamsterArray.some((h) => h[prop] == value)) return true;
    element.innerHTML = message;
    return false;
  },
  hasWinValue: (wins, hamsterArray, element) =>
    ev.hasValue(
      wins,
      "wins",
      hamsterArray,
      element,
      "The hamster with the most wins is not in the array."
    ),
  hasLoseValue: (defeats, hamsterArray, element) =>
    ev.hasValue(
      ...
      hamsterArray,
      element,
      "The hamster with the most defeats is not in the array."
    ),

  objectHasId: (object, element) => {
    if (object && object.id) return true;
    element.innerHTML =
      'Must respond with an object that has an id property: { id: "id from database here" }';
    return false;
  },
  objectHasProp: (object, prop, element) => {
    if (object && object.hasOwnProperty(prop)) return true;
